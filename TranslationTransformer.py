    ...
    def get_tgt_mask(self, tgt_len: int) -> torch.tensor:
        """
            ...
            Arg:
                tgt_len: Target
        """
        return nn.Transformer.generate_square_subsequent_mask(tgt_len).bool()  # Corrected mask size
...
class TestTranslationTransformer(unittest.TestCase):
    ...
    def test_get_tgt_mask(self):
        ...
        self.assertEqual(mask.shape, (tgt_len, tgt_len))
    ...
    def test_decode_output_shape(self):
        ...
        out = self.model.decoder(tgt, memory, tgt_mask)
...
def train_loop(model, optimizer, criterion, train_dataloader):
    ...
    for batch in tqdm(train_dataloader):
        ...
        tgt_mask = model.get_tgt_mask(dec_tokens.size(1)).to(device)
...
def translate(eng_sentence: str, model: TranslationTransformer,
    ...
    enc_input_tokens = tokenizer_en.tokenize(eng_sentence)
    src_tokens = torch.cat([
        torch.tensor([tokenizer_en.special_tokens["SOS"]], dtype=torch.int64),
        torch.tensor(enc_input_tokens, dtype=torch.int64),
        torch.tensor([tokenizer_en.special_tokens["EOS"]], dtype=torch.int64),
        torch.tensor([tokenizer_en.special_tokens["PAD"]], dtype=torch.int64).repeat(max_en_len - len(enc_input_tokens) - 2)
    ]).unsqueeze(0).to(device) # Shape: [1, src_len]


    # Initialize target sequence with [SOS]
    tgt_tokens = torch.tensor([tokenizer_fr.special_tokens["SOS"]], dtype=torch.int64).unsqueeze(0).to(device)  # Shape: [1, 1]

    # Autoregressive decoding
    tgt_tokens=perdict(model, src_tokens, tgt_tokens, max_tgt_len,tokenizer_fr.special_tokens["EOS"])

    # Decode the token sequence to a French sentence
    fr_ids = tgt_tokens[0].cpu().tolist()
    fr_sentence = tokenizer_fr.detokenize(fr_ids)
